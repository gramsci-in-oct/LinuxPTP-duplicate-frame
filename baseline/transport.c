...
#include "msg.h"
#include "time.h"
//globl_parameter copy_frame times
...
/*维护一个队列，存入sync时间戳以供顺序发送*/
int hwts_transport_node(struct hw_timestamp *hwts,struct hw_timestamp_queue_node *hwts_queue_node)
{
	hwts_queue_node -> type = hwts -> type;
       
	hwts_queue_node -> ts.ns = hwts -> ts.ns;

	hwts_queue_node -> sw.ns = hwts -> sw.ns;
