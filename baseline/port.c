...
static int port_ignore(struct port *p, struct ptp_message *m,int key)
{
...
/**/
int msg_process_Timestamp(struct ptp_message *msg)
{
        int type;
        type=msg_type(msg);
	//printf("msg->hwts.ts %f\n",tmv_dbl(msg->hwts.ts));
                switch (type) {
                    case 11://announce
                        return 0;
	            case 0://SYNC
                    {
                        hwts_transport_node(&msg->hwts,&msg->hwts_queue_node);
                       // printf("!!!!!!sync: m->hwts_queue_node %" PRId64 "\n",msg->hwts_queue_node.ts.ns);
                        if(hw_queue.size == copy_count){
                    //        printf("queue in full!");
                            return 0;
                        }
                        hw_queue.data[hw_queue.rear]=msg->hwts_queue_node;
                        hw_queue.rear = (hw_queue.rear+1)%copy_count;
                        hw_queue.size++; 
    
                        for(int i=0;i<hw_queue.size;i++){
                        //printf("!!sync:hw_queue.data[%d] %" PRId64 "\n",i,hw_queue.data[i].ts.ns);  
                        }       
		        return 0;
                     }
	             case 2://PDELAY_REQ
			...
			return 0;
	             ...
	             case 3://PDELAY_RESP 
		     {
		      return 0;
	             ...
	             case 8://FOLLOW_UP:
                     {
                       //  printf("fup queue size:%d\n",hw_queue.size);

                          msg->follow_up.preciseOriginTimestamp = tmv_to_Timestamp(hw_queue.data[hw_queue.front].ts);
                   
		      // printf("follow_up.preciseOriginTimestamp:%" PRId32 "\n",msg->follow_up.preciseOriginTimestamp.nanoseconds);
                       //  printf("hw_queue.data[m->hwts_queue.front].ts:%" PRId64 "\n",hw_queue.data[hw_queue.front].ts.ns);
                        
			hw_queue.front = (hw_queue.front+1)%copy_count;
                        hw_queue.size--;
...
/*0=sync 8=followup 1=reply 9=resp*/
			  ...
			  enum transport_event event,int type)
{
	...
	if (!msg_unicast(msg)) {
		...
		{
		    //取时间戳
                    msg_process_Timestamp(msg);
           	//msg_presend处理的是msg中已有的sync时间戳，需要再进行大端小端变换
		    msg->follow_up.preciseOriginTimestamp.seconds_lsb = htonl(msg->follow_up.preciseOriginTimestamp.seconds_lsb);
		    msg->follow_up.preciseOriginTimestamp.seconds_msb = htons(msg->follow_up.preciseOriginTimestamp.seconds_msb);
		    msg->follow_up.preciseOriginTimestamp.nanoseconds = htonl(msg->follow_up.preciseOriginTimestamp.nanoseconds);
		    msg->data.buffer[999]=msg->data.buffer[999]+1;//标明是第几个包，其实里面有本来不用这个
                    cnt = transport_sendto(p->trp, &p->fda, event, msg);
		...
		if (q==9){msg->data.buffer[999]=key;}
             cnt = transport_sendto(p->trp, &p->fda, event, msg);
            }
	}
