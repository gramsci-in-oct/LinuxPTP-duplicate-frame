...
//duplicate frame
#define Duplicate_Table_Size 20
int key = 0;
int64_t former_diff = 999999999999999999;
//int64_t T12=0;
int64_t T34=0;
#define C 16
int64_t former_diff2 = 999999999999999999;
struct hw_queue hw_queue = {.front = 0, .rear = -1, .size = 0};
					...
					struct ptp_message *req)
{
	...
	if (respond)
		port_prepare_and_send(ingress, rsp, TRANS_GENERAL,1);//reponse
	msg_put(rsp);
	return respond;
}
			     ...
			     Integer8 sync_interval)
{
	...
	//T12 = tmv_to_nanoseconds(tmv_sub(t2,t1));
	printf("\n\nt2:%li,t1:%li,t2-t1:%li\n\n",t2,t1,tmv_sub(t2,t1));
	//if (tmv_to_nanoseconds(tmv_sub(t2,t1))>2000000000){t2= timestamp_to_tmv(origin_ts);
	//	t1=ingress_ts;}
	switch (p->state) {
 ...
 */
// 状态机函数，用于处理端口的同步（SYNC）和转发（FUP）消息
// 根据事件和消息更新端口状态并同步时钟。
			 ...
			 struct ptp_message *m)
{
	...
	case SF_EMPTY:
		...
		case SYNC_MISMATCH:
			//matters
			printf("there goes the sync process.");
			msg_get(m);
			p->last_syncfup = m;
			p->syfu = SF_HAVE_SYNC;
		...
		break;
//第一次进sync没改状态 导致fup进的也是SYNC
	case SF_HAVE_SYNC:
		//printf("have sync now\n");
		switch (event) {
		case SYNC_MISMATCH:
		...
		case FUP_MATCH:
            syn = p->best_sync;
			m = p->best_fup;
			printf("sync:%li,fup:%li\n",syn->hwts.ts, timestamp_to_tmv(m->ts.pdu));
...
// 发送Pdelay请求消息（Peer-to-Peer 延迟请求消息）
...
int port_delay_request(struct port *p)
{
	...
	if (port_prepare_and_send(p, msg, TRANS_EVENT,0)) //delay_request
    {
...
int port_tx_announce(struct port *p, struct address *dst)
{
	...
	err = port_prepare_and_send(p, msg, TRANS_GENERAL,1);//announce
	if (err) {
...
// 发送同步（SYNC）消息和（如果需要的话）跟随消息（FOLLOW_UP）
int port_tx_sync(struct port *p, struct address *dst)
{
	...
	msg->data.buffer[999] = 0;
	if (dst) {
		msg->address = *dst;
		msg->header.flagField[0] |= UNICAST;
		msg->header.logMessageInterval = 0x7f;
	}
	err = port_prepare_and_send(p, msg, event,0);//sync
	if (err) {
	...
	err = port_prepare_and_send(p, fup, TRANS_GENERAL,0);//followup
	if (err) {
...
static int process_delay_req(struct port *p, struct ptp_message *m)
{
	...
	msg->data.buffer[999]=0;
	err = port_prepare_and_send(p, msg, TRANS_GENERAL,0);//delay_resp
	if (err) {
...
{	
	...
	printf("!!there goes the delay resp!!\n"); 
	c3 = correction_to_tmv(m->header.correction);
	t3 = req->hwts.ts;
	t4 = timestamp_to_tmv(m->ts.pdu);
	t4c = tmv_sub(t4, c3);
	printf("T4:%li,T3:%li\n",t4,t3);
	T34 = tmv_to_nanoseconds(tmv_sub(t4,t3));
	printf("\nt4-t3:%li\n\n",T34);
	...
	msg_put(p->best_resp);//bu que ding !
}
...
void process_follow_up(struct port *p, struct ptp_message *m)
{
	...
	printf("theres goes the fup process.\n");
...
void process_sync(struct port *p, struct ptp_message *m)
{
	...
	//printf("theres goes the sync process.\n");
	if (!msg_unicast(m) &&
...
// 处理与端口相关的事件
// 执行以下操作：
// 根据不同的文件描述符（fd_index）处理超时事件或链路状态变化。
// 处理接收到的消息，进行必要的处理并返回相应的事件。
static enum fsm_event bc_event(struct port *p, int fd_index)
{
	...
	//if(msg->header.tsmt==0){printf("%li\n",tmv_to_nanoseconds(msg->hwts.ts));}
	int64_t current_ts=0;
	uint16_t seq_number = msg->header.sequenceId;
	//对时间戳进行提取，转为ns
	if(tsmt==0)//sync
	{
	current_ts=tmv_to_nanoseconds(msg->hwts.ts);
	//printf("sync: %li of key %i\n",current_ts,key);
	}		
	if(tsmt==8|tsmt==9)//
	{
	current_ts=tmv_to_nanoseconds(timestamp_to_tmv(msg->ts.pdu));
	}

	//if(tsmt==8){printf("fup : %li of key %i\n",current_ts,key);}
	struct dup_table *dupt = &p->seq_table;

	//printf("seq_NUMBER:%li\n",seq_number);

    // 冗余包判断逻辑
	if(tsmt==9|tsmt==0){
		if (dupt->count < 80000) {
			// 还有空间，直接新增
			//printf("dupt->count:%u  tsmt=%u  data=%u  ts=%lu   \n",dupt->count,tsmt,key,current_ts);
			dupt->rec[dupt->count].tsmt = tsmt;
			dupt->rec[dupt->count].seq = seq_number;
			dupt->rec[dupt->count].data = key;
			dupt->rec[dupt->count].ts = current_ts;
			dupt->rec[dupt->count].msg = msg;
			//printf("save sucessfully\n");
			dupt->count++;} 
		else {
			msg_put(dupt->rec[80000-1].msg);
			// 没有空间，采用 FIFO 替换策略
			memmove(&dupt->rec[0], &dupt->rec[1], (80000 - 1) * sizeof(struct records));
			dupt->rec[80000 - 1].tsmt = tsmt;
			dupt->rec[80000 - 1].seq = seq_number;
			dupt->rec[80000 - 1].data = key;
			dupt->rec[80000 - 1].ts = current_ts; 
		
			dupt->rec[80000 - 1].msg = msg;
			 }
	}
	if(tsmt==8){
	...
	if(tsmt==8)//followup
	{
/*初始化间隔，当小于当前间隔时对最佳消息进行更新;不是第一个间隔时删除上一个的地址;传递消息指针和时间戳*/
	if(key==1){
		former_diff =  999999999999999999;
		}

		for(int i =0;i<dupt->count;i++)
		{
			//printf("i's TIMESTAMP:%li\n",dupt->rec[i].ts);
			if(dupt->rec[i].tsmt==0&dupt->rec[i].seq==seq_number&dupt->rec[i].data==key)
			{
				diff=(dupt->rec[i].ts)-current_ts;

				printf("diff_between_sync and fup %li of key:%i\n",diff,dupt->rec[i].data);
				if(diff<former_diff)
					{
					if(former_diff<999999999999){
					//printf("del sync and fup\n");					
					msg_put(p->best_sync);
					msg_put(p->best_fup);}
					former_diff=abs(diff);
					//printf("there goes the upd!\n");
					msg_get(dupt->rec[i].msg);
					p->best_sync = dupt->rec[i].msg;
					
					p->best_sync->hwts.ts=nanoseconds_to_tmv(dupt->rec[i].ts);
					//printf("best sync=%li\n",tmv_to_nanoseconds(p->best_sync->hwts.ts));
					msg_get(msg);
					p->best_fup = msg;				
					p->best_fup->ts.pdu=msg->ts.pdu;
					}
			}
		}
	...
	if(key==1){
			...
			former_diff2 = 999999999999999999;}

		//printf("ts of resp:%li\n",current_ts);
		for(int i =0;i<dupt->count;i++)
		{
			
			if(dupt->rec[i].tsmt==1&dupt->rec[i].seq==seq_number&dupt->rec[i].data==key)
			{
				
				diff2=current_ts-dupt->rec[i].ts;
				//diff2=(diff2+1000000000)%1000000000;
				printf("diff2 between req and resp %li of key:%i\n",diff2,dupt->rec[i].data);
				if(diff2<former_diff2)
				{
...
/*ptp里req已经写成了队列*/
					...
					req=dupt->rec[i].msg;
					req->hwts.ts=nanoseconds_to_tmv(dupt->rec[i].ts);
					if(former_diff2<999999999999){msg_put(p->best_resp);}					
					msg_get(msg);			
					p->best_resp = msg;
					
					p->best_resp->ts.pdu=msg->ts.pdu;
...
//仅仅在全部到达后才进行处理
	...
	case DELAY_RESP:
		if(key==C){
		process_delay_resp(p, p->best_resp);
		}
		break;
...
//不能释放掉可能用到的指针
	if(tsmt!=8&tsmt!=9&tsmt!=0&tsmt!=1){msg_put(msg);}
	//printf("msg_process_ended.\n");
	return event;
}
...
int msg_process_Timestamp(struct ptp_message *msg,struct port *p,uint8_t id)
{

                         return 0;
	            
}
/*保存消息类型 时间戳 序列号 组内位置/key 消息指针*/
int port_prepare_and_send(struct port *p, struct ptp_message *msg,
			  enum transport_event event,int type)
{
	int cnt=0;
	int q;
	...
	q=msg_type(msg);
	uint8_t id =msg->header.sequenceId;
	...
	if (!msg_unicast(msg)) {
		cnt = transport_sendto(p->trp, &p->fda, event, msg);
			...
			{
            for(int i=0;i<C;i++){
		msg->data.buffer[999]=msg->data.buffer[999]+1;
	     	cnt = transport_sendto(p->trp, &p->fda, event, msg);
		    struct dup_table *dupt = &p->seq_table;
			int tsmt=msg->header.tsmt;
			int64_t ts;
			ts=tmv_to_nanoseconds(msg->hwts.ts);
			printf("REQTS:%li,seq:%i,data:%li,tsmt:%li\n",ts,id,msg->data.buffer[999],tsmt);
			//
			if (dupt->count< 80000) {
		// 还有空间，直接新增
			//printf("dupt->count:%u\n",dupt->count);
			...
			dupt->rec[dupt->count].tsmt = tsmt;
			dupt->rec[dupt->count].seq = id;
			dupt->rec[dupt->count].data = msg->data.buffer[999];
			dupt->rec[dupt->count].ts = ts;
			dupt->rec[dupt->count].msg = msg;
			dupt->count++;} 
			else {
			// 没有空间，采用 FIFO 替换策略
				...
				memmove(&dupt->rec[0], &dupt->rec[1], (80000 - 1) * sizeof(struct records));
				dupt->rec[80000 - 1].tsmt = tsmt;
				dupt->rec[80000 - 1].seq = id;
				dupt->rec[80000 - 1].data = msg->data.buffer[999];
				dupt->rec[80000 - 1].ts = ts; 
				dupt->rec[80000 - 1].msg = msg;
			  ...
			  struct ptp_message *req, Enumeration16 error_id)
{
	...
	err = port_prepare_and_send(ingress, msg, TRANS_GENERAL,0);//error
	msg_put(msg);
	return err;
}
...
/*dup frame*/
	...
	memset(&p->seq_table,0,sizeof(p->seq_table));
