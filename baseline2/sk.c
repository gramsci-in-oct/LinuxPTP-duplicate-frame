	       ...
	       struct address *addr, struct hw_timestamp *hwts, int flags)
{
	...
	for (cm = CMSG_FIRSTHDR(&msg); cm != NULL; cm = CMSG_NXTHDR(&msg, cm)) {
        ...
        unsigned char *received_data = (unsigned char *)msg.msg_iov->iov_base;
        size_t received_data_len = msg.msg_iov->iov_len;
	if (cnt > 0&received_data[44]>0&received_data[44]<C+1) {

		hwts->data=received_data[44];

	}
	if (cnt == 55&received_data[54]>0&received_data[54]<C+1) {	
		hwts->data=received_data[54];
		//printf("data recvd:%u\n",hwts->data);
	}
	if (addr)
