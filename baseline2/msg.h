...
#define PDELAY_RESP_FOLLOW_UP 0xA//10
#define ANNOUNCE              0xB
...
struct hw_timestamp_queue_node {
	enum timestamp_type type;
	//硬件时间戳和软件时间戳
	tmv_t ts;
	tmv_t sw;
...
// 定义队列
struct hw_queue{
    struct hw_timestamp_queue_node data[copy_count]; // shuzu
    int front;
    int rear;
    int size;
} ;
...
struct ptp_message {
	...
	struct {
		/**
		 * Contains the time stamp from the packet data in ahwts
    ...
    struct hw_timestamp_queue_node hwts_queue_node;
	struct hw_queue hwts_queue;
	/**
